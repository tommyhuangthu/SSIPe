...
if(@ARGV != 4){
  print "This script is used to convert psiblast xml format to MSA file\n";
  print "Usage: xml2msa.pl -in psiblast.xml -out psiblast.msa\n";
  exit(-1);
}
...
while(<$fh>){
  ...
  while ($_ =~ /<Hit>(.*?)<\/Hit>/sg){
    ...
    while ($hit =~ /<Hsp>(.*?)<\/Hsp>/sg){
      ...
      #if($nonspacechar/$query_len < 0.3){
      #  next;
      #}
      # 2) reject the hit sequence if it is quite similar to the query
      #elsif($nonspacechar/$query_len > 0.98 && $sameaa/$nonspacechar > 0.98){
      #  next;
      #}
